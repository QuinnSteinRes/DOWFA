...
def tornado_table(names, src_values, prcc_values, title, outfile=None):
    """
    Generate tabular tornado data as CSV/DataFrame.
    
    Produces a ranked table with:
    - Rank (by |SRC|)
    - Parameter name
    - SRC value
    - PRCC value
    - Direction indicator (+/-)
    - Relative importance (% of max |SRC|)
    
    Parameters
    ----------
    names : list of str
        Parameter names
    src_values : array-like
        SRC coefficients
    prcc_values : array-like
        PRCC coefficients
    title : str
        Table title (used in output filename)
    outfile : str, optional
        Output CSV path. If None, only returns DataFrame.
    ...
    prcc_values = np.asarray(prcc_values)
    
    # Sort by absolute SRC (descending)
    order = np.argsort(np.abs(src_values))[::-1]
    
    max_abs_src = np.max(np.abs(src_values))
    
    rows = []
    for rank, idx in enumerate(order, 1):
        rel_importance = (np.abs(src_values[idx]) / max_abs_src * 100) if max_abs_src > 0 else 0
        rows.append({
            "Rank": rank,
            "Parameter": names[idx],
            "SRC": round(src_values[idx], 4),
            "PRCC": round(prcc_values[idx], 4),
            "Direction": "+" if src_values[idx] > 0 else "-",
            "Rel_Importance_%": round(rel_importance, 1)
        })
    ...
    if outfile:
        ...
        print(f"Saved tornado table: {outfile}")
...
def run_R3_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
                      save_csv=True, make_plot=True, make_table=True, outprefix="./"):
    """
    ...
    if make_table:
        table_file = f"{outprefix}R3_tornado_table_{anaesthetic}_GWP{gwp_type}.csv"
        tornado_table(names, SRC, PRCC,
                     f"R3 MSWI ({anaesthetic.upper()}, GWP-{gwp_type})",
                     outfile=table_file)
...
def run_R4_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
    ...
    if make_table:
        table_file = f"{outprefix}R4_tornado_table_{anaesthetic}_GWP{gwp_type}.csv"
        tornado_table(names, SRC, PRCC,
                     f"R4 HCWI ({anaesthetic.upper()}, GWP-{gwp_type})",
                     outfile=table_file)
...
def run_R5_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
    ...
    if make_table:
        table_file = f"{outprefix}R5_tornado_table_{anaesthetic}_GWP{gwp_type}.csv"
        tornado_table(names, SRC, PRCC,
                     f"R5 Plasma ({anaesthetic.upper()}, GWP-{gwp_type})",
                     outfile=table_file)
...
def run_R6_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
                      save_csv=True, make_plot=False, make_table=False, outprefix="./"):
    """
...
def run_R7_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
    ...
    if make_table:
        table_file = f"{outprefix}R7_tornado_table_{anaesthetic}_GWP{gwp_type}.csv"
        tornado_table(names, SRC, PRCC,
                     f"R7 AC+MSWI ({anaesthetic.upper()}, GWP-{gwp_type})",
                     outfile=table_file)
...
def run_R8_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
    ...
    if make_table:
        table_file = f"{outprefix}R8_tornado_table_{anaesthetic}_GWP{gwp_type}.csv"
        tornado_table(names, SRC, PRCC,
                     f"R8 AC+HCWI ({anaesthetic.upper()}, GWP-{gwp_type})",
                     outfile=table_file)
...
def run_R9_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
    ...
    if make_table:
        table_file = f"{outprefix}R9_tornado_table_{anaesthetic}_GWP{gwp_type}.csv"
        tornado_table(names, SRC, PRCC,
                     f"R9 AC+Plasma ({anaesthetic.upper()}, GWP-{gwp_type})",
                     outfile=table_file)
...
def run_R10_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
    ...
    if make_table:
        table_file = f"{outprefix}R10_tornado_table_{anaesthetic}_GWP{gwp_type}.csv"
        tornado_table(names, SRC, PRCC,
                     f"R10 AC+Onsite ({anaesthetic.upper()}, GWP-{gwp_type})",
                     outfile=table_file)
...
def run_R11_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
    ...
    if make_table:
        table_file = f"{outprefix}R11_tornado_table_{anaesthetic}_GWP{gwp_type}.csv"
        tornado_table(names, SRC, PRCC,
                     f"R11 Insitu Plasma ({anaesthetic.upper()}, GWP-{gwp_type})",
                     outfile=table_file)
...
def run_all_routes_sensitivity(seed=42, N=10000, anaesthetic='des', gwp_type='100',
    ...
    df_r3, sum_r3 = run_R3_sensitivity(seed, N, anaesthetic, gwp_type, 
                                       save_csv, make_plot, make_table, outprefix)
    print(df_r3)
