...
def print_uncertainty_table(route_name, sd_des, sd_sev, sd_iso):
    """Print a formatted table of Monte Carlo uncertainties"""
    print("\n" + "="*70)
    print(f"Monte Carlo Uncertainties (Standard Deviations) - {route_name}")
    print("="*70)
    print(f"{'Anaesthetic':<20} {'Std Dev (kgCO2e/kg)':<30}")
    print("-"*70)
    print(f"{'Desflurane':<20} {sd_des:<30.4f}")
    print(f"{'Sevoflurane':<20} {sd_sev:<30.4f}")
    print(f"{'Isoflurane':<20} {sd_iso:<30.4f}")
    print("="*70)
        ...
        try:
            ...
            if gwp_selection in gwp_categories:
                ...
                if all(item is not None for item in (rho, gfm, carbon_economy, cf4_gwp)):
                    ...
                    # Print formatted table instead of individual values
                    print("\n" + "="*80)
                    print(f"Legacy Stock Emissions - {variable_selection.upper()} - {gwp_selection}")
                    print("="*80)
                    print(f"{'Route':<30} {'Std Dev (kgCO2e/kg)':<30}")
                    print("-"*80)
                    print(f"{'Release':<30} {sd_cv_1:<30.4f}")
                    print(f"{'MSWI':<30} {sd_cv_2:<30.4f}")
                    print(f"{'PWI':<30} {sd_cv_3:<30.4f}")
                    print(f"{'Plasma':<30} {sd_cv_4:<30.4f}")
                    print(f"{'Transport (Network)':<30} {sd_cv_5:<30.4f}")
                    print("="*80 + "\n")
    ...
    def calculate(self):
        try:
            ...
            if gwp_selection in gwp_categories:
                    ...
                    # Print formatted tables for all routes
                    print("\n" + "="*80)
                    print("WASTE VOLATILE ANAESTHETICS - MONTE CARLO UNCERTAINTIES")
                    print("="*80)
                    
                    print_uncertainty_table("R6: Continuous Release", sd_cv_6d, sd_cv_6s, sd_cv_6i)
                    print_uncertainty_table("R7: AC capture with MSWI", sd_cv_7d, sd_cv_7s, sd_cv_7i)
                    print_uncertainty_table("R8: AC capture with HCWI", sd_cv_8d, sd_cv_8s, sd_cv_8i)
                    print_uncertainty_table("R9: AC capture with perfluorinated waste", sd_cv_9d, sd_cv_9s, sd_cv_9i)
                    print_uncertainty_table("R10: AC capture with on-site incineration", sd_cv_10d, sd_cv_10s, sd_cv_10i)
                    print_uncertainty_table("R11: In-situ plasma destruction", sd_cv_11d, sd_cv_11s, sd_cv_11i)
